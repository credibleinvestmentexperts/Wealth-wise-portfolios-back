...
const toggleHasFees = asyncHandler(async (req, res) => {
  const { hasFees } = req.body;
  const { id } = req.params;
  ...
  if (hasFees === "true") {
    ...
    hasFeesBoolean = false;
  } else {
    res.status(400);
    throw new Error("hasFees must be 'true' or 'false'.");
  }
  ...
  user.hasFees = hasFeesBoolean;
  await user.save();

  res.status(200).json({
    message: `hasFees has been set to ${hasFeesBoolean}`,
    user: {
      _id: user._id,
      hasFees: user.hasFees,
    },
  });
});

const adjustUserFees = asyncHandler(async (req, res) => {
  ...
  if (!user.hasFees) {
    res.status(403);
    throw new Error("Fees access not enabled for this user.");
  }

  if (!["add", "deduct"].includes(operation)) {
    res.status(400);
    throw new Error("Operation must be 'add' or 'deduct'.");
  }
  ...
  if (operation === "add") {
    ...
    if (user.fees < amount) {
      res.status(400);
      throw new Error("Insufficient fee balance to deduct.");
    }
    user.fees -= amount;
  }
  ...
  await user.save();
  res.status(200).json({
    message: `Fees ${operation}ed successfully`,
    fees: user.fees,
    userId: user._id,
  });
});
...
const sendComposedEmail = asyncHandler(async (req, res) => {
  ...
  if (!subject || !send_to || !reply_to || !message) {
    res.status(400);
  ...
  try {
    ...
    // Call simplified version of sendEmail
    await sendEmail(
      subject,
      send_to,
      sent_from,
